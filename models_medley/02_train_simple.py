...
def train(fold,
    ...
    idx_train = pd.read_json(os.path.join(OUTPUT_PATH, 'index_simple_train{:02d}.json'.format(fold)))
    idx_val = pd.read_json(os.path.join(OUTPUT_PATH, 'index_simple_validate{:02d}.json'.format(fold)))
    ...
    with open(os.path.join(OUTPUT_PATH, modelid, '{:02d}'.format(fold), 'model_spec.pkl'),
              'wb') as fd:
        pickle.dump(model_spec, fd)

    # save the model definition
    modeljsonfile = os.path.join(OUTPUT_PATH, modelid, '{:02d}'.format(fold), 'model.json')
    model_json = model.to_json()
    with open(modeljsonfile, 'w') as json_file:
        json.dump(model_json, json_file, indent=2)

    # Construct the weight path
    weight_path = os.path.join(OUTPUT_PATH, modelid, '{:02d}'.format(fold), 'model.h5')
    ...
    history_checkpoint = os.path.join(OUTPUT_PATH, modelid, '{:02d}'.format(fold),
                                      'history_checkpoint.pkl')
    cb.append(LossHistory(history_checkpoint))

    history_csvlog = os.path.join(OUTPUT_PATH, modelid, '{:02d}'.format(fold), 'history_csvlog.csv')
    ...
    # Save history
    with open(os.path.join(OUTPUT_PATH, modelid, '{:02d}'.format(fold), 'history.pkl'), 'wb') as fd:
    ...
    test_idx = pd.read_json(os.path.join(OUTPUT_PATH, 'index_simple_test{:02d}.json'.format(fold)))['id']
